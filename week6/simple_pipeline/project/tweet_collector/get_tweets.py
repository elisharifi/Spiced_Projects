...
cursor=tweepy.Paginator(method=client.search_recent_tweets, query=search_query, tweet_fields=['id', 'text', 'author_id', 'public_metrics']).flatten(limit=200)
#for tweet in cursor:
    #print(tweet.text+"\n")
'''   
...
tweet_db = client_m.tweet_db
for tweet in cursor:
    #print("we are inside the loop")
    tweet_db.tweets.insert_one({"tweet":tweet.text, "tweet_time":tweet.created_at})